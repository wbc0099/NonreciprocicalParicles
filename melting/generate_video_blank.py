...
for folder in tqdm(matching_folders):
    ...
    kba = initial_parameter.values[19]
    N = Na + Nb
    ...
    for ii in tqdm(range(0, len(dat_files))):
        ...
        # 保存图像
        plt.savefig(os.path.join(temp_path0, str(ii) + ".png"), bbox_inches='tight', pad_inches=0)
        plt.close()

    video_name_no_path = [str(int(N[0])), str(int(Na[0])), str(int(Nb[0])), "{:.2f}".format(float((Na / Nb)[0])),str(kba[0])]
